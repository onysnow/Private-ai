...
{"finding_id":"STRUCT-0009","audit_object":"backend/app/core/security_controls.py","audit_pass":"folder-structure","severity":"MATERIAL","finding":"460 lines bundling three independent concerns under one module: request-size/envelope validation (RequestLimitPolicy, validate_request_envelope), rate limiting (FixedWindowRateLimiter), and security-audit-log read/retention (SecurityAuditLogger, read_security_audit, summarize_security_audit, *_retention). None of the three depend on each other's internals; they are grouped only because they are all \"security-adjacent.\"","precise_locations":["backend/app/core/security_controls.py"],"required_correction":"Split into app/core/request_limits.py, app/core/rate_limiter.py, and app/core/audit_log.py, each independently unit-testable without importing the other two. Mechanical, low-risk -- same class of refactor as STRUCT-0002 but much smaller.","reviewer_type":"AI_ASSISTED","status":"OPEN"}
...
{"finding_id":"STRUCT-0012","audit_object":".github/workflows/backend-postgres-ci.yml","audit_pass":"ci-workflows","severity":"BLOCKING","finding":"backend-postgres-ci.yml is the ONLY workflow in this repo that invokes pytest (confirmed: codeql.yml, docker-build.yml, and frontend-ci.yml contain no pytest step at all). It is also the exact workflow broken by task #35's still-unresolved 0-job dispatch failure. The practical consequence: the backend's 216-test suite has not been executed by CI on any push or PR to this repo for as long as task #35 has been open, including both PR #49 (issue #36) and PR #50 (this cycle's fixes) -- both merged on green CodeQL/docker-build/frontend checks alone. Every regression check this cycle happened only because a human-directed session manually synced files to a local venv and ran pytest by hand; that is not a substitute for an enforced CI gate.","precise_locations":[".github/workflows/backend-postgres-ci.yml"],"required_correction":"Treat task #35 as blocking, not merely diagnostic. Until it's fixed, this repo's actual quality gate for backend changes is whoever happens to run pytest manually before merging -- worth stating plainly rather than assuming \"CI is green\" means the backend tests passed. Once fixed, confirm it actually runs and blocks merge (branch protection / required check) rather than just existing.","reviewer_type":"AI_ASSISTED","status":"OPEN"}
```
